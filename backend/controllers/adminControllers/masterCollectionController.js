...
const dummyData = [
    ...
    { id: uuidv4(), model: 'NISSAN MICRA Petrol MT', variant: 'manual', type: 'car', brand: 'nissan' },
...
export const getCarModelData = async (req, res, next) => {
    try {
        ...
        return res.status(200).json(availableVehicleModels);
    } catch (error) {
        console.log("Error al obtener datos, devolviendo datos dummy:", error);
        // En caso de error, usar el error handler o devolver datos dummy
        return next(errorHandler(500, "Error al obtener datos del servidor"));
    }
};